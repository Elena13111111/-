...
# pip install pandas -через Терминал
# pip install openpyxl --через Терминал
...
def show_menu(access_role, login):
    while True:
        ...
        print(current_menu)
        try:                # любой на выбор пункт из списка
            ...
            if access_role == STUDENT_ACCESS and chosen_item == 1:
                ...
                show_student_info()
        except Exception as e:           # если введи не сущ-ий пункт
            print('Такого пункта в меню не существует')


def load_students():
    if os.path.exists(STUDENTS_FILE_NAME):
        xlsx_data = pandas.ExcelFile(STUDENTS_FILE_NAME)    #читаем наш файл
        for sheet in xlsx_data.sheet_names:    # подгружаем имя студента и его страница
            df1 = pandas.read_excel(xlsx_data, sheet)
            marks = df1.values.tolist()        # оценки
            students[sheet] = list(      #студент, их страница = лист оценок
                map(                       #приобразуем строку в дату в екселе
                    lambda el:
                    (
                        datetime.datetime(
                            year=int(el[0][:4]),  # год до 4 числа
                            month=int(el[0][5:7]),  # месяц с -по
                            day=int(el[0][8:10]),  # день с -по
                            hour=int(el[0][11:13]),  # часы с -по
                            minute=int(el[0][14:16]),  # минуты с -по
                            second=0
                        ),
                        el[1]),
                    marks
...
def run_study_program():
    load_students()    #подгрузка предыдущих данных
