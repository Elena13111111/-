#логическое ядро / программа для работы с тестами /сервис- для логических задач

import traceback

from utils import Utils  #из файла utils импортируем класс Utils
import datetime
...
class TestService:
    ...
    def __init__(self, repo): # репозиторий файл
        self.repo = repo
        # self.name = name
        # self.marks = marks
...
# начало теста
    @staticmethod
    def start_test(student):   # начало теста для студента
        right_answers = 0      # изначальное ответов 0
        for i in range(1, 6):  #5 примеров рандомных
            try:
                question, answer = Utils.generate_random_test()   # рандомный тест находится в файле utils class Utils
                print(f'Реши пример: {question}')
                user_answer = int(input('Ваш ответ: '))
                if answer == user_answer:   #если ответ правильный
                    right_answers += 1    #ответ прибавляеться +1
            except Exception as e:        #если ответ не соответствует Числу
                print('Вы ввели ерунду и потеряли шанс получить баллы')

        mark = right_answers * 2       #оценка = ответы * 2
        student.marks.append((datetime.datetime.now(), mark))   #студента оценка добавлеться в (дата сейчас и оценка)
        print(f'Твоя отметка: {mark}')

    # начало викторины
    @staticmethod
    def start_viktorina(student):  # начало викторины для студента
        vsego = Utils.start_viktorina(student)
        # print(kol, " верных ответов из ", vsego)

    # для студента-Посмотреть свои отметки
    @staticmethod
    def show_student_marks(student, students):  # принимает имя студента и словарь students
        if student.name in students.keys():
            to_print = f" Итоговые оценки Студента  = {student.name}: \n"
            for counter, mark in enumerate(student.marks):          # итоговые оценки
                to_print += f' {counter}.{mark[1]}  ->  {mark[0]}\n'
            print(f"{'=' * 80}\n{to_print}\n{'=' * 80}")
        else:
            print(f"{'=' * 80}")

#для админа, показать всек отметки студентов
    @staticmethod
...
# вычисление ср.балл всех студентов
    @staticmethod
    ...
    def students_menu(self, student, students):
        ...
        if chosen_item == "1":
            ...
            TestService.show_student_marks(student, students)
        elif chosen_item == "3":
            TestService.start_viktorina(student)
        elif chosen_item == "10":
 ...
 #  меню для админа
    ...
    # общее меню/ нестатический метод
        ...
        while True:
            ...
            try:
                if current_role == Utils.STUDENT_ACCESS:
                    res = self.students_menu(student, students)   # рез для выхода
                elif current_role == Utils.ADMIN_ACCESS:
                    res = self.admins_menu(students)
                else:         # для заканчивания
                    res = self.RETURN_STATUS
                if res == self.RETURN_STATUS:  # если результат равен статус СТОП,
                    return                  # тогда разрыв цепочки
            except Exception as e:
                traceback.print_exc()
                print('Такого пункта в меню не существует')

# если не правильный логин или пароль
